...
Spreadsheet.prototype.worksheets = function(fn){
  ...
  return this.load(function(err,atom){
    ...
    spreadsheet.sheetCount = parseInt(atom['openSearch:totalResults']);
    spreadsheet.startIndex = parseInt(atom['openSearch:startIndex']);
    // Create a worksheet from each entry
    var entries = Array.isArray(atom.entry) ? atom.entry : [atom.entry];
    entries.forEach(function(entry,i){
      fn(null,new Worksheet(spreadsheet,entry.id.match(/\w+$/)[0],i+1,entry.title["#"], new Date(entry.updated) ));
    });
  });
}
...
Spreadsheet.prototype.worksheetArray = function(fn){
  ...
  return this.load(function(err,atom){
    ...
    var entries = Array.isArray(atom.entry) ? atom.entry : [atom.entry];
    var sheets = [], len = entries.length;
    for(var i=0; i<len; i+=1){
    	var entry = entries[i];
    	sheets.push(new Worksheet(spreadsheet,entry.id.match(/\w+$/)[0],i+1,entry.title["#"], new Date(entry.updated) ));
    };
  ...
  return this.load(function(err,atom){
    ...
    if(toString.call(id) == '[object Number]') {
    	if(id>0 && id<=entries.length) {
        	...
        	return fn(null, new Worksheet(spreadsheet,entry.id.match(/\w+$/)[0],id,entry.title["#"], new Date(entry.updated) ));
    	}
    }else{
	    ...
	    for(var i=0; i<len; i+=1){
	    	...
	    	if( eId == id ) {
	    		return fn(null, new Worksheet(spreadsheet,eId,i+1,entry.title["#"], new Date(entry.updated) ));
	    	}
	    };
    }
...
*  @constructor
*/
function Worksheet(spreadsheet,id,index,title, updated){
  if( !spreadsheet )
...
function cellValue(obj) {
    ...
    for (var key in obj) if (hasOwnProperty.call(obj, key)) return obj;
    return null;
  };
