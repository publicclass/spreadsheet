...
Spreadsheet.prototype.worksheets = function(fn){
  ...
  return this.load(function(err,atom){
    ...
    entries.forEach(function(entry,i){
      fn(null,new Worksheet(spreadsheet,entry.id.match(/\w+$/)[0],i+1,entry.title["#"], new Date(entry.updated), parseInt(entry['gs:rowCount']), parseInt(entry['gs:colCount']) ));
    });
  });
}
...
*  @return Itself for chaining.
*/
Spreadsheet.prototype.sheetArray = function(fn){
  var spreadsheet = this;
  return this.load(function(err,atom){
    if( err ) return fn(err);
    spreadsheet.author = atom.author;
    ...
    var sheets = [];
    entries.forEach(function(entry,i){
    	sheets.push(new Worksheet(spreadsheet,entry.id.match(/\w+$/)[0],i+1,entry.title["#"], new Date(entry.updated), parseInt(entry['gs:rowCount']), parseInt(entry['gs:colCount']) ));
    });
...
*  @constructor
*/
function Worksheet(spreadsheet,id,index,title, updated, rowCount, colCount){
  if( !spreadsheet )
